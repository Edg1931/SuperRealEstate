...
| **App** | `ARCore.VoiceCommandController` | drag into `RealEstateApp.voiceCommandController`; set `RealEstateApp.targetPlatform` to your device. `OnActionBlocked`/`OnError` for fallbacks |
| **App** | `ARCore.VoiceCaptureController` | the STT/TTS loop: drag the `VoiceCommandController` in, bind `pushToTalk` to a hold-to-talk button/pinch. Picks the platform speech backend automatically (real STT+TTS on Android/Galaxy XR; manual/log fallback in Editor — call `FeedTranscript("measure this room")` to test the whole loop now). Bind `OnPartialTranscript`/`OnFinalTranscript`/`OnListeningChanged` to a caption/mic HUD. **Android manifest:** add `RECORD_AUDIO` permission + request it at runtime. iOS/visionOS STT/TTS need a native plugin (SFSpeechRecognizer / AVSpeechSynthesizer) — the interface is ready |
...
| **UI/Tools** | `ARCore.RadialToolMenuView` | the core nav palette; assign a `TMP_FontAsset`, set `targetPlatform`, bind `summonAction` to a gesture/button (palm-up / menu). Unavailable tools auto-dim per device |
| **UI/Tools** | `ARCore.ToolMenuActionBridge` | drag the `RadialToolMenuView` + `SceneAppActions` in — routes Measure/Identify/Finishes to actions and raises `OnToolModeRequested` for Stage/RemoveWall/Landscape/Notes. This is what makes the menu *do* things |
| **Input** | `ARCore.CommitFeedback` | drag `SpatialPointerInput` in; assign a click `AudioClip` + `AudioSource`; wire `OnCommit` to platform haptics (controller rumble / hand-pinch). Fires on every pinch/tap |
| **Renderers/Property** | (already added) `ARRender.PropertyOverlayRenderer` | drag into `RealEstateApp.propertyOverlayRenderer`; tick `provideTestLocation` + set `testLatitude/testLongitude` until device GPS is wired, so "show comps" works |
